...
const FALLBACK_FAVICON_URL = "/noor-honey-favicon.svg";
const LEGACY_BRAND_NAMES = new Set(["সততা ব্র্যান্ড"]);
...
export function mergeSettings(
  ...
  if (LEGACY_BRAND_NAMES.has(String(merged.brand_name).trim())) {
    merged.brand_name = DEFAULT_SETTINGS.brand_name;
  }
...
function upsertHeadLink(rel: string, href: string, attrs: Record<string, string> = {}) {
...
function getFaviconUrl(s: Pick<SiteSettings, "favicon_url" | "logo_url">) {
  return s.favicon_url?.trim() || s.logo_url?.trim() || FALLBACK_FAVICON_URL;
}

function getFaviconType(url: string) {
...
function setFavicons(s: Pick<SiteSettings, "favicon_url" | "logo_url">) {
  const href = getFaviconUrl(s);

  upsertHeadLink("icon", href, {
    type: getFaviconType(href),
  });
